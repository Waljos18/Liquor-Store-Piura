...
6. **Desarrollar app móvil:** Aplicación Android (Android Studio + Kotlin + Firebase) para gestión desde dispositivos móviles
...
- **App Móvil Android (Kotlin + Firebase) para gestión:** Se prioriza el sistema web y POS. La app Android puede desarrollarse en una fase posterior (semanas 15-18) si se requiere.
...
│   (React.js)    │     │(Android/Kotlin/  │     │  (Electron)     │
│                 │     │  Firebase)       │     │                 │
└────────┬────────┘     └────────┬────────┘     └────────┬────────┘
         │                       │                       │
         └───────────────────────┼───────────────────────┘
...
#### 4.2.5 App Móvil (Android)
- **IDE:** Android Studio
- **Lenguaje:** Kotlin
- **Firebase:** Authentication, Firestore o Realtime Database, Cloud Messaging (FCM), Storage
- **Arquitectura:** MVVM, Jetpack Compose o XML para UI
- **HTTP:** Retrofit/OkHttp o Ktor para API REST del backend
- **Async:** Kotlin Coroutines
- **Código de barras:** ML Kit Barcode Scanning o ZXing
...
- **App Móvil:** Firebase (Auth, Firestore/Realtime DB, FCM, Storage) – tier gratuito
- **Almacenamiento de Archivos:** AWS S3 o local; Firebase Storage para app móvil
...
5. **Desarrollador POS (1):** Desarrollo de aplicación POS
6. **Desarrollador Android (1):** Desarrollo de app móvil (Android Studio + Kotlin + Firebase) – opcional para fases posteriores
7. **QA/Tester (1 part-time):** Pruebas y aseguramiento de calidad
8. **Diseñador UX/UI (1 part-time):** Diseño de interfaces (enfocado en simplicidad)

**Nota:** Equipo enfocado en entregar funcionalidades core (Web + POS + IA) en 14 semanas. App móvil Android (Kotlin + Firebase) queda como funcionalidad opcional para fases posteriores.
...
**Nota:** App móvil Android (Kotlin + Firebase) se puede desarrollar en fase posterior o como opcional. Para 14 semanas, el enfoque está en web + POS.
...
**Nota:** Desarrollo optimizado con trabajo paralelo donde sea posible. App móvil Android (Kotlin + Firebase) queda como funcionalidad opcional para fases posteriores.
...
| Desarrollador Android | 1 | Desarrollo de app móvil (Kotlin + Firebase) |
...
- App móvil Android (Kotlin + Firebase) queda como funcionalidad opcional para fases posteriores (no incluida en presupuesto).
...
5. **App móvil Android (Kotlin + Firebase):** Gestión desde cualquier lugar; notificaciones push (FCM)
